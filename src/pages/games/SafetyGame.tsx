...
const SCENARIOS: Scenario[] = [
  ...
  { emoji: '🔄', text: 'อัปเดตระบบและแอปจากร้านค้าอย่างเป็นทางการ', safe: true, why: 'การอัปเดตช่วยแก้ช่องโหว่และควรดาวน์โหลดจากแหล่งที่เชื่อถือได้' },
  { emoji: '📶', text: 'ทำธุรกรรมสำคัญผ่าน Wi-Fi สาธารณะที่ไม่รู้จัก', safe: false, why: 'เครือข่ายสาธารณะอาจไม่ปลอดภัย ควรหลีกเลี่ยงการส่งข้อมูลสำคัญ' },
  { emoji: '👤', text: 'ตั้งค่าบัญชีให้เห็นข้อมูลส่วนตัวเฉพาะคนที่รู้จัก', safe: true, why: 'การจำกัดผู้ชมช่วยลดการเปิดเผยข้อมูลส่วนตัวเกินความจำเป็น' },
  { emoji: '🔢', text: 'ใช้รหัสผ่าน 123456 เหมือนกันทุกบัญชี', safe: false, why: 'รหัสผ่านเดาง่ายและใช้ซ้ำทำให้หลายบัญชีเสี่ยงพร้อมกัน' },
  { emoji: '📸', text: 'ขออนุญาตเพื่อนก่อนโพสต์รูปที่มีเพื่อนอยู่ในภาพ', safe: true, why: 'ภาพใบหน้าเป็นข้อมูลของบุคคลอื่น จึงควรได้รับความยินยอมก่อนเผยแพร่' },
  { emoji: '🧑‍💻', text: 'ให้รหัสยืนยัน OTP กับคนที่โทรมาอ้างว่าเป็นเจ้าหน้าที่', safe: false, why: 'OTP เป็นข้อมูลลับ เจ้าหน้าที่จริงไม่ควรขอรหัสนี้จากเรา' },
  { emoji: '🚩', text: 'บล็อกและรายงานบัญชีที่คุกคามหรือส่งข้อความไม่เหมาะสม', safe: true, why: 'การบล็อก เก็บหลักฐาน และบอกผู้ใหญ่ช่วยหยุดการคุกคามออนไลน์' },
  { emoji: '📎', text: 'เปิดไฟล์แนบจากผู้ส่งที่ไม่รู้จักเพราะชื่อไฟล์ดูน่าสนใจ', safe: false, why: 'ไฟล์แนบอาจมีมัลแวร์ ควรตรวจผู้ส่งและไม่เปิดไฟล์ที่น่าสงสัย' },
];
    ...
    <div className="game-page compact-game-page" style={{ maxWidth: 620, textAlign: 'center' }}>
      ...
      {done ? (
        <div className="card" style={{ padding: '2rem' }}>
          <div style={{ fontSize: '3rem' }}>{score >= Math.ceil(order.length * 0.8) ? '🏆' : score >= Math.ceil(order.length * 0.6) ? '😃' : '💪'}</div>
          <h2>ได้ {score}/{order.length} คะแนน!</h2>
          <p>{score >= Math.ceil(order.length * 0.8) ? 'สุดยอด! รู้เท่าทันภัยออนไลน์' : 'ทบทวนเหตุผลแต่ละข้อ แล้วลองใหม่นะ'}</p>
