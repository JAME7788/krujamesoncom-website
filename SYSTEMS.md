# 📚 Kru James Soncom Portal — System Documentation

> ระบบสารสนเทศเพื่อการจัดการเรียนการสอนวิชาเทคโนโลยี/วิทยาการคำนวณ  
> โรงเรียนบ้านคลองมดแดง • ปีการศึกษา 2569

---

## 🎯 ภาพรวมระบบ

### ความเสถียรส่วนกลาง 10 กันยายน 2569

SearchBar ใช้ native dialog และ App มี PageBoundary ภายใน Layout ซึ่งรีเซ็ตตาม pathname รายละเอียด error แสดงเฉพาะ DEV ดู [ผลตรวจ](docs/shared-ui-stability-2026-09-10.md)

### ศูนย์ใช้งานหน้าแรก

`WorkspaceHome` ที่ `/` รวมทางลัดและหมวดตามบทบาทจาก `portalDirectory` โดยใช้บัญชีเดิม การค้นหากลางอ่านเกมจาก `gamesCatalog` ทั้งหมด หน้าแนะนำเดิมอยู่ `/about` ไม่มีการเปลี่ยนสิทธิ์ปลายทางหรือข้อมูลคะแนน ดู [ผลตรวจและขอบเขตการปรับ](docs/portal-upgrade-2026-09-09.md)

### บันทึกตรวจเกมล่าสุด: 9 กันยายน 2569

รอบพัฒนาเพิ่มเติมปรับ 7 เกม: เกมจับคู่มีระดับ 4/6/8 คู่และพักเวลา ส่วนเกมหลายข้อ 6 เกมส่งคะแนนรวมตอนจบรอบพร้อมคะแนนเต็มจริงและกันกดซ้ำ ใช้ `gameRoundGuard`, `gameRecords` และ `memoryEngine` ดู [รายละเอียดการพัฒนาและผลทดสอบ 610 ข้อ](docs/game-improvements-2026-09-09.md) ยังไม่ได้รับรองการเล่นครบทุกด่านหรือออนไลน์หลายเครื่องจริง

การเชื่อมคะแนนเกมปรับให้ยืนยันกิจกรรมด้วย transaction ส่งซ้ำเมื่อขั้นใดล้มเหลว แยกหลักฐานตามวิชา/หน่วย และรักษาคะแนนที่ครูกรอกไว้ ดู [ผลตรวจการเชื่อมคะแนน](docs/game-score-connection-2026-09-09.md) การเรียก `recordGame(score, activityKey, maxScore)` ต้องส่งคะแนนเต็มจริงเมื่อทราบ; ไม่ส่งหมายถึงคะแนนดิบ ไม่ใช่เต็ม 100 เกณฑ์ K/P/A เดิมไม่เปลี่ยน และยังไม่มีคิวออฟไลน์ถาวรข้ามการปิดหน้า

ตรวจเปิดครบ 37 เส้นทางเกมบนจอคอมและมือถือ แก้ปัญหาตัวหน่วงเวลา คะแนนซ้ำ ป๊อปอัปบทเรียน และหน้าล้นขอบ รายงานนี้แยกการตรวจเปิดหน้าออกจากการทดลองเล่นจริง ไม่ได้ยืนยันการเล่นครบทุกด่านหรือหลายเครื่องออนไลน์ ดู [ขอบเขตและผลตรวจเกม](docs/game-audit-2026-09-09.md)

เกมที่ใช้ `useGameTimers` ต้องเรียก `clear()` เมื่อเริ่มใหม่หรือเปลี่ยนด่าน ส่วน hook จะยกเลิก callback เมื่อ unmount และเมื่อ round key เปลี่ยน การบันทึกด้วย `useGameProgress` รองรับ `activityKey` เพื่อกันซ้ำแยกภารกิจ และให้เหรียญเมื่อบริการบันทึกสำเร็จเท่านั้น

เว็บไซต์การเรียนรู้แบบครบวงจรสำหรับ ป.1 - ม.3 ที่รวม:
- **คอร์สเรียน** ตามมาตรฐาน สสวท. (10+ คอร์ส + AI 3 คอร์ส)
- **ระบบเก็บคะแนน** K/P/A อัตโนมัติตามตัวชี้วัด ว 4.1 + ว 4.2
- **เครื่องมือเสริมการสอน** ครบครันสำหรับครู
- **เกม + กิจกรรม** ฝึกทักษะในเว็บ
- **ระบบ Admin** จัดการทุกอย่างได้

**Tech stack:** React 19 + TypeScript + Vite + Framer Motion + Firebase + Vercel Functions

## อัปเดตระบบวัดผลและส่งออก ปพ.5 & เกมฝึกเมาส์ ว.PA — 22 สิงหาคม 2569

- **ยกระดับเกมฝึกเมาส์ (Mouse Practice Pro)**: พัฒนารองรับประเด็นท้าทาย ว.PA ชั้น ป.1 (11 คน) ครอบคลุม 5 โหมด (คลิกซ้ายเดี่ยว, ดับเบิลคลิกเปิดกล่อง, คลิกขวา, ลากและวางจัดหมวดหมู่ฮาร์ดแวร์, และโหมดทดสอบจับเวลามาตรฐาน ว.PA Pre/Post Test พร้อมสถิติความแม่นยำและคอมโบ)
- **ปรับปรุงเกณฑ์รูบริก P 4 ระดับมาตรฐานใหม่**: ดีมาก (24-30 / เขียว), ดี (18-23 / ฟ้า), พอใช้ (12-17 / เหลือง-ส้ม), ปรับปรุง (<12 / แดง-ชมพู แทนคำว่า "ไม่ผ่าน"), และแสดง `—` (เทา) สำหรับช่องที่ยังไม่ประเมิน
- **เพิ่มระบบ Roster Reconciliation**: ป้องกันนักเรียนตกหล่น (ป.1 มีครบ 11 คนเสมอ) ไม่ว่าจะดึงจาก Cloud หรือโหลดในเครื่อง
- **ระบบส่งออกผลการเรียน ปพ.5 & Master Spreadsheet**: บริการ `gradeExportService.ts` คำนวณสถิติวัดผล ($\bar{X}$, S.D., เกรด 0-4, ร้อยละเกรด $\ge 3.00$, อัตราผ่าน)
- ส่งออกไฟล์ Spreadsheet (.xls / .csv) รองรับภาษาไทยสมบูรณ์ มีขอบเส้น สีสัน และสรุปผลสัมฤทธิ์
- ส่งออก Master Grade Sheet รวม 9 ห้อง 111 คน (ป.1 - ม.3) ในไฟล์เดียวสำหรับส่งต่อฝ่ายวิชาการ
- ส่งออกไฟล์โครงสร้างคอลัมน์มาตรฐานสำหรับระบบทะเบียนโรงเรียน (SchoolMIS / SGS)
- เพิ่ม `OfficialPp5PrintLayout.tsx` สำหรับพิมพ์แบบ ปพ.5 มาตรฐาน A4 แนวนอน พร้อมสถิติ ($\bar{X}$, S.D., แจกแจงเกรด 0-4) และช่องลงนาม 3 ฝ่าย (ครูผู้สอน, หัวหน้าวัดผล, ผอ.โรงเรียน)
- เพิ่ม `StudentGradeSlipPrintLayout.tsx` สำหรับพิมพ์ใบแจ้งผลการเรียนรายบุคคล (ปพ.6) 2 คน/แผ่น A4 พร้อมรอยปรุตัดแจกผู้ปกครอง
- เพิ่มศูนย์ `OfficialGradeExportModal.tsx` และปุ่มเข้าถึงใน `GradeBook.tsx` และ `AdminDashboard.tsx`
- **นโยบายความปลอดภัยของฐานข้อมูล 100%**: ไม่มีการลบ ดัดแปลง หรือเขียนทับข้อมูลดิบใน Firebase และ LocalStorage
- ผ่านชุดทดสอบ Vitest 492/492 tests และ production build ผ่าน 100%

## อัปเดตข้อมูลพร้อมสอน — 3 สิงหาคม 2569

- รายชื่อกลาง 111 คน 9 ห้อง; `students` และ `progress` ตรงกันโดยไม่มีบัญชีซ้ำ
- กำหนดการ ป.1-ม.3 รวม 360 คาบ, ร่างบันทึกหลังสอน 78 รายการ, คลังข้อสอบ 550 ข้อ และงานเชื่อมตัวชี้วัด 26 งาน
- ศูนย์คาบเรียนวันนี้อิงตารางสอนจริงเพื่อเลือกห้องปัจจุบัน/ห้องถัดไป และแยกหลักฐานประถมเป็น `main` กับมัธยมเป็น `cs`
- แบบประเมินตั้งต้นใช้สัดส่วนผู้เรียนระดับกลาง 22-27% ต่อห้อง และต้องให้ครูยืนยันจากหลักฐานจริงก่อนใช้รายงานผล
- แบบวิเคราะห์ผู้เรียนที่ครูยืนยันแล้วเก็บใน Firebase และได้รับการป้องกันไม่ให้สคริปต์ seed เขียนทับ
- โปรไฟล์ความสามารถเชื่อมไปยังสมรรถนะ คุณลักษณะ อ่าน–คิด–เขียน ค่านิยม และสมรรถนะเทคโนโลยี โดยคงคะแนน K/P/A หลังแผนเป็นร่างจนกว่าครูจะตรวจหลักฐาน
- ทุกห้อง ป.1-ม.3 สร้างแบบ K/P/A และบันทึกหลังสอนตามจำนวนคาบที่สอนจริงจาก `TeachingSession`
- ปฏิทินกลางข้ามวันงดเรียนและถูกใช้ร่วมกันในตารางสอน การนับในคาบ ศูนย์คาบเรียน และหลังแผน
- ทุกหน่วยของ 17 รายวิชา รวม 73 หน่วย มีกิจกรรมอย่างน้อย 3 กิจกรรม
- `learningEvidence` ไม่ถูกเติมย้อนหลังตามคำสั่งครู; ระบบจะเพิ่มหลักฐานเมื่อเกิดกิจกรรมจริงเท่านั้น
- Firebase Auth สำหรับครูถูกสร้างแล้ว พร้อม role `admin`; รอครูตั้งรหัสผ่านจากอีเมลและทดสอบก่อนปิด legacy fallback

คำสั่งดูแลข้อมูล: `npm run data:audit`, `npm run content:audit`,
`npm run teaching-data:seed`, `npm run students:clean` และ `npm run teacher-auth:setup`

ผลตรวจล่าสุด: Vitest 381/381, game QA 370 checks, browser smoke 36/36,
ห้อง 3D desktop/mobile และ production build ผ่านทั้งหมด

## อัปเดตระบบฝั่งครู — 31 กรกฎาคม 2569

รุ่น `589b85e` รวมการทำงานฝั่งครูให้ใช้ข้อมูลชุดเดียวกันตั้งแต่คาบเรียนจนถึงผลการเรียน

```mermaid
flowchart LR
  A[คาบเรียนวันนี้] --> B[เช็กชื่อ]
  A --> C[สไลด์ บทเรียน เกม ควิซ]
  A --> D[การบ้านและงานภายนอก]
  B --> E[หลักฐาน A]
  C --> F[หลักฐาน K/P]
  D --> F
  E --> G[สมุดคะแนน K/P/A]
  F --> G
  G --> H[รายงานและบันทึกหลังสอน]
```

### ระบบที่เพิ่ม

- `TeacherClassroomHub` — ควบคุมคาบ เช็กชื่อ เครื่องมือสอน หลักฐาน และหลังสอนในหน้าเดียว
- `technologyLessonPlans` — แผน ป.1-6 ชั้นละ 40 แผน แผนละ 50 นาที
- `teachingSessionService` — วันที่ตามแผน วันที่สอนจริง และสถานะรายคาบ
- `learningEvidenceService` — หลักฐานกลาง K/P/A จากทุกกิจกรรม
- `QuestionBankManager` — คลังข้อสอบ 10 ข้อแบบแบ่งระดับและวิเคราะห์รายข้อ
- `HomeworkManager` — งานผ่านลิงก์ คะแนน K และรูบริก P
- `AuditLogViewer` — ประวัติการแก้คะแนน เนื้อหา แผน งาน และข้อสอบ
- `contentService` — draft/publish, version history และ restore

### Firestore collections ที่เกี่ยวข้อง

`teachingSessions`, `learningEvidence`, `questionBank`, `courseVersions`,
`teacherAuditLogs`, `teacherProfiles`, `lessonRecords`, `homeworkAssignments`,
`homeworkSubmissions`, `grades`, `progress` และ `attendance`

### สถานะการตรวจสอบ

- 154/154 tests ผ่าน
- lint และ production build ผ่าน
- ตรวจหน้าเว็บ 27 เส้นทาง ไม่มีหน้าเสีย ไม่มี horizontal overflow และไม่มี console error/warning

---

## 🗺️ Routing Map

| Route | หน้า | Auth | คำอธิบาย |
|-------|------|------|---------|
| `/` | Home | Public | หน้าแรก + ประกาศข่าวสาร + Leaderboard |
| `/login` | Login | Public | เลือกห้อง → เลือกชื่อ |
| `/courses` | Courses | Public (gated) | คลังคอร์สเรียน filter ตามชั้น |
| `/curriculum/:gradeId/unit/:unitNo` | UnitDetail | Student | หน้าเรียนสไลด์/ควิซ/กิจกรรม |
| `/curriculum/:gradeId/:idx` | IndicatorDetail | Student | รายละเอียดตัวชี้วัด |
| `/dashboard` | Dashboard | Student | สถิติส่วนตัว + ปฏิทิน + กราฟ |
| `/report-card` | ReportCard | Student | สมุดรายงานผลการเรียน (พิมพ์ได้) |
| `/resources` | Resources | Public | คลังเครื่องมือ 107+ รายการ |
| `/games` | Games Hub | Public | รวมเกมฝึกจาก `gamesCatalog` |
| `/games/{mouse-practice,keyboard-practice,...}` | Games | Public | เกมแต่ละเกม |
| `/admin` | AdminDashboard | Admin client gate | แผงควบคุมครูแบบแบ่งหมวด |
| `*` | NotFound | Public | 404 page |

---

## 🔐 1. Authentication System

### ผู้ใช้งาน 2 ประเภท
1. **Student (นักเรียนในโรงเรียน)** — login ด้วยห้อง + ชื่อ + เลขที่ที่ต้องตรง roster 2569 จึงได้คะแนน
2. **External (ผู้ทดลองภายนอก)** — กรอกเฉพาะชื่อ เข้าดูบทเรียน/เกมได้ ไม่เช็กชื่อ ไม่ส่งงาน ไม่สร้าง K/P/A
3. **Admin (ครู)** — มีบัญชี Firebase Auth, custom role และ `teacherProfiles`; legacy fallback ยังเปิดชั่วคราวจนกว่าครูจะตั้งรหัสผ่านและทดสอบการเข้าใช้สำเร็จ

### ไฟล์หลัก
- `src/context/AuthContext.tsx` — student session + partner (โหมดนั่งคู่)
- `src/services/authAdmin.ts` — admin session 8 ชั่วโมง
- `src/components/AdminGate.tsx` — modal login admin
- `src/services/userAccessService.ts` — กติกากลางว่าบัญชีใดเก็บคะแนนได้
- `src/services/externalVisitorService.ts` — สถิติผู้ทดลองแยกจากข้อมูลการเรียน

### Features
- **โหมดนั่งคู่** (Pair Mode) — login 2 คนพร้อมกัน บันทึกคะแนนให้ทั้งคู่
- **Session persistence** — localStorage
- **Auto-redirect** — ถ้าไม่ login → ส่งไป `/login`

---

## 👥 2. Student Roster (รายชื่อนักเรียน 2569)

### ข้อมูลในระบบ
- **111 คน • 9 ห้อง** (ป.1 - ม.3)
- มาจาก `รายชื่อนักเรียนทั้งหมด_โรงเรียนบ้านคลองมดแดง2569.xlsx`

### ไฟล์หลัก
- `src/data/students2569.ts` — built-in roster (immutable)
- `src/services/rosterService.ts` — CRUD override layer (localStorage)

### CRUD ผ่าน Admin Tab "จัดการนักเรียน"
- ➕ เพิ่มนักเรียน
- ✏️ แก้ไข (เลขที่, ชื่อ, รหัส, emoji)
- 🔄 ย้ายห้อง
- 🗑️ ลบ
- 🔢 จัดเรียงเลขที่
- 📤 Export CSV
- ⏮️ รีเซ็ตกลับเป็นค่าเดิม

---

## 📚 3. Course/Curriculum System

### โครงสร้างหลักสูตร
**12 คอร์ส** + 3 AI courses:
- ป.1 - ป.6 (ว 4.2 วิทยาการคำนวณ)
- ม.1 - ม.3 cs (ว 4.2 วิทยาการคำนวณ)
- ม.1 - ม.3 design (ว 4.1 ออกแบบเทคโนโลยี)
- AI ป.1-3, AI ป.4-6, AI ม.1-3

### Access Control
นักเรียนเห็นเฉพาะคอร์สของชั้นตัวเอง:
```
ป.5 → p5 + ai-p4-6
ม.2 → m2-cs + m2-design + ai-m1-3
Admin → ทุกคอร์ส (16+)
```

### ไฟล์หลัก
- `src/data/curriculum.ts` — โครงสร้างคอร์ส (units, indicators, lessons)
- `src/data/unitContent.ts` — สไลด์เนื้อหา (text + image)
- `src/data/unitExtras.ts` — extras (intro, videos, fun, quiz, articles)
- `src/data/richSlides.ts` — สไลด์ Rich Format ครบ 73 หน่วย รวม 426 สไลด์ พร้อมระบบเติมภาพ คำสรุปตามวัย คำอธิบาย และกิจกรรมสั้น
- `src/pages/Courses.tsx` — หน้ารายการคอร์ส
- `src/pages/Curriculum.tsx` — modal รายละเอียด
- `src/pages/UnitDetail.tsx` — หน้าเรียน (เนื้อหา + ควิซ)

### Content per Unit
แต่ละหน่วยมี 6 tabs:
1. **ภาพรวมบทเรียน** — intro + topics
2. **สไลด์เนื้อหา** — Rich slides หรือ JPG (500+ หน้า)
3. **วิดีโอประกอบ** — YouTube search queries
4. **กิจกรรมสนุก** — เกมจากภายนอก (Code.org, Scratch...)
5. **แบบทดสอบ** — quiz มี K score
6. **อ่านเพิ่มเติม** — articles จาก สสวท., scimath

---

## 🎓 4. Grade Book (เก็บคะแนน K/P/A)

### โครงสร้างคะแนน — มาตรฐานไทย 100 คะแนน
- **คะแนนเก็บ 70** = K (60%) + P (25%) + A (15%) ต่อตัวชี้วัด
- **สอบ 30** = ป. ปลายภาค 30 / ม. กลางภาค 15 + ปลายภาค 15
- **คะแนนรวม = 100** → ตัดเกรด 0-4 ตามเกณฑ์ไทย

### ตัวชี้วัด (Indicators)
- **ป.1-6**: ว 4.2 (4-5 ตัวชี้วัด/ชั้น)
- **ม.1-3 วิทยาการคำนวณ**: ว 4.2 (4 ตัวชี้วัด/ชั้น)
- **ม.1-3 ออกแบบ**: ว 4.1 (5 ตัวชี้วัด/ชั้น)

### Auto-sync (K/P/A → กิจกรรมในเว็บ)
ครูกดปุ่ม **"นำเข้า K/P/A จากเว็บ"**:
- **K** = คะแนนควิซ best score × normalize
- **P** = skill points (เกม×3 + วิดีโอ×2 + สไลด์/บทความ×1) → ดี/ปานกลาง/พอใช้
- **A** = เข้าเรียนตรงเวลาตามตาราง ≥ 2 วัน

### ไฟล์หลัก
- `src/services/gradeService.ts` — K/P/A calculation + storage
- `src/components/GradeBook.tsx` — UI ตารางคะแนน
- `src/pages/ReportCard.tsx` — สมุดรายงานพิมพ์ได้

### Features
- ✅ ครูแก้ค่าเองได้ทุกช่อง
- ✅ Subject tabs สำหรับ ม.X (cs vs dt แยกเก็บ)
- ✅ Export CSV ตามรูปแบบไฟล์ Excel เดิม
- ✅ Firebase sync (optional)
- ✅ Linkage panel แสดงตัวชี้วัด ↔ หน่วยการเรียน

---

## 📊 5. Progress Tracking

### บันทึกทุกการเรียน
- **อ่านสไลด์** (≥ 0.8 วินาที = นับ)
- **กดวิดีโอ/เกม/บทความ**
- **ทำควิซ** (เก็บ history + best score)
- **เวลาที่เข้า** (สำหรับคำนวณ attendance)

### ไฟล์หลัก
- `src/services/progressService.ts` — Firebase source of truth + in-memory cache ต่อ session
- Auto-sync ทุก click → ครูเห็นเรียลไทม์

### Storage Strategy
- **Primary**: Firebase Firestore
- **Session cache**: in-memory เท่านั้น จึงไม่ค้างข้ามนักเรียนเมื่อ logout
- **Limit**: 50 quiz attempts + 100 activity logs ล่าสุด

---

## 🏆 6. Achievements/Badges

### Badges 18 ตัว — 4 tiers
- **Bronze** (ทองแดง) — เริ่มต้น
- **Silver** (เงิน) — ระดับกลาง
- **Gold** (ทอง) — ขั้นสูง
- **Diamond** (เพชร) — มาสเตอร์

### หมวด
- 📖 **Reading** — อ่านสไลด์ (1, 10, 50, 100, 500)
- 🎯 **Quiz** — ทำควิซ (first, perfect, avg 80%, 10x)
- 🎮 **Activity** — กิจกรรม (1, 25, 100)
- 🌟 **Streak/Completion** — จบหน่วย (1, 5, 10, all)
- ⭐ **Special** — pair learner, all-rounder

### ไฟล์หลัก
- `src/services/achievementService.ts` — 18 badges + check logic
- `src/components/AchievementsBadge.tsx` — modal + trophy icon ใน navbar
- Auto-check ทุก 5 วินาที + แจ้งเตือนเมื่อปลดล็อก

---

## 📣 7. Announcements (ประกาศข่าวสาร)

### 4 ประเภท
- ℹ️ Info • ⚠️ Warn • 🚨 Urgent • 🎉 Celebration

### Features
- ✅ Filter ตามห้อง (เฉพาะ ป.5 หรือทุกห้อง)
- 📌 Pin ไว้ด้านบน
- ⏰ Expire date
- 👁️ นักเรียนปิดประกาศได้ (dismiss state)

### ไฟล์หลัก
- `src/services/announcementService.ts`
- `src/components/AnnouncementManager.tsx` — Admin tab
- `src/components/AnnouncementBanner.tsx` — แสดงบน Home + Dashboard

---

## 📅 8. Calendar (ปฏิทินกิจกรรม)

### 5 ประเภท Event
- 📝 การบ้าน • 📋 สอบ • 🎉 กิจกรรม • 🌴 วันหยุด • 👥 ประชุม

### Features
- 📅 วันที่ + เวลา + ห้อง + ลิงก์
- 🔔 Dashboard นักเรียนแสดง 5 events ล่วงหน้า 14 วัน
- ⏰ "วันนี้!" / "พรุ่งนี้" / "อีก X วัน" (สีตามความใกล้)

### ไฟล์หลัก
- `src/services/calendarService.ts`
- `src/components/CalendarManager.tsx` — Admin tab

---

## 🕐 9. Schedule (ตารางสอน)

### Class Slot
- Classroom + Day (0-6) + Start time + End time + Subject

### Features
- 🔴 "กำลังเรียน" ตามเวลาจริง (today slots)
- ⏰ Auto-attendance — กิจกรรมในเวลาเรียน = present
- 🔄 ครูแก้ตารางได้ผ่าน Admin tab
- 🗂️ เช็กชื่อด่วนเก็บใน `attendance`; เช็กชื่อตามตารางอ่านหลักฐานถาวรจาก `progress.inClassDays`

### ปรับการเข้าเรียนย้อนหลัง

- ตรวจแผนการปรับโดยไม่เขียนข้อมูล: `npm run attendance:backfill`
- ปรับจริงถึงวันก่อนหน้าวันปัจจุบัน: `npm run attendance:backfill -- --apply`
- ตรวจว่ารายชื่อ วันเข้าเรียน และสถานะมาเรียนครบ: `npm run attendance:verify`
- สคริปต์เติมเฉพาะวันที่ห้องมีคาบตามตาราง ไม่สร้างวันเสาร์ อาทิตย์ หรือวันไม่มีคาบ
- การปรับย้อนหลังไม่แจก XP และไม่สร้าง learning evidence ปลอม

### กำหนดการสอนปี 2569

- ภาคเรียนที่ 1 เริ่ม 5 พฤษภาคม 2569; ภาคเรียนที่ 2 เริ่ม 2 พฤศจิกายน 2569
- ป.1-6 มี `teachingSessions` ใน Firebase ชั้นละ 40 แผน รวม 240 แผน แผนละ 1 ชั่วโมง
- วันที่ตามแผนคำนวณจากวันเปิดภาคเรียนและวันประจำคาบของแต่ละชั้น
- ตรวจแผนก่อนเขียน: `npm run teaching-schedule:backfill`
- เขียนกำหนดการขึ้น Firebase: `npm run teaching-schedule:backfill -- --apply`
- ตรวจจำนวนแผนและวันที่: `npm run teaching-schedule:verify`

### ไฟล์หลัก
- `src/data/schedule.ts`
- `src/services/adminService.ts` — attendance calculation
- `scripts/backfill-scheduled-attendance.mjs` — dry run, apply และ verify ข้อมูลย้อนหลัง

---

## 📈 10. Leaderboard

### Ranking Logic
1. คะแนนรวม (totalPoints) — desc
2. กิจกรรม (totalActivities) — desc
3. สไลด์ (totalSlides) — desc

### Features
- 🏆 แสดง Top 10 + filter ตามห้อง
- 🥇🥈🥉 Crown/Medal สำหรับ 3 อันดับ
- 🔄 Auto-refresh ทุก 10 วินาที
- 📊 Mini stats ของแต่ละคน

### ไฟล์หลัก
- `src/services/leaderboardService.ts`
- `src/components/LeaderboardSection.tsx` — แสดงบน Home

---

## 🎨 11. Rich Slides

### Layout Types 5 แบบ
1. **Cover** — ภาพใหญ่ + emoji ใหญ่
2. **Standard** — ข้อความ + bullets + รูปข้าง
3. **Split** — ซ้ายข้อความ / ขวารูป
4. **Comparison** — VS เปรียบเทียบ 2 ฝั่ง
5. **Quote** — คำคมใหญ่

### Features
- 🎨 7 themes สี (blue/green/orange/purple/pink/yellow/red)
- 💡 Callouts (tip/warn/fun/quote)
- 💻 Code blocks (Python, Scratch, Pseudocode)
- 🖼️ รูปจาก Unsplash CDN

### Content
- **426 Rich Slides** ครบ 73 หน่วย และรวมสไลด์เสริมตามตัวชี้วัดในหน้าเรียน
- AI courses ครบ 3 ระดับ + main curriculum หน่วยสำคัญ

### ไฟล์หลัก
- `src/data/richSlides.ts`
- `src/components/RichSlideViewer.tsx`

---

## 🎮 12. Mini Games (เกมฝึก)

### 24 เกมใน catalog
| เกม | ฝึก | ระดับ |
|-----|------|------|
| 🖱️ Mouse Practice | ใช้เมาส์ | ป.1-3 |
| ⌨️ Keyboard Practice | พิมพ์ | ป.1-6 |
| 🧩 Algorithm Sorter | คิดเป็นขั้นตอน | ป.4-ม.3 |
| 🔢 Binary Game | เลขฐาน 2 | ม.1-3 |
| 🃏 Memory Match | ความจำ | ป.1-6 |
| 🔍 Pattern Game | Pattern Recognition | ป.3-ม.3 |
| 🤖 **Coding Maze** | เขียนโปรแกรม block-based | ป.2-ม.3 |
| 🐍 **Snake** | Loop + Conditional | ทุกระดับ |
| 🐞 **Bug Catcher** | Debug + Reaction | ทุกระดับ |

### Features
- 🏆 Best score ใน localStorage
- 📊 Live stats + combo system
- 📱 Responsive (touch + keyboard)

### ไฟล์
- `src/pages/games/Games.tsx` — hub
- `src/pages/games/{*}.tsx` — แต่ละเกม

---

## 📚 13. Resources Library

### 107+ Resources จัด 7 หมวด
1. 💻 **Programming** — Scratch, PictoBlox, **Code.org A-F + Express**, Hour of Code themes (Frozen, Star Wars, Dance, Minecraft, Flappy, Sprite Lab, App Lab, Game Lab, CSD, CSP)
2. 🧩 **Computational** — CS Unplugged, Bebras, CodingThailand
3. 📊 **Data** — Canva, Datawrapper, Gapminder
4. 🛡️ **Safety** — Interland, Think Digital, Password Game
5. 🤖 **AI** — Teachable Machine, Quick Draw, AI for Oceans, NN Playground
6. 🎨 **Design** — Tinkercad, Figma, Excalidraw
7. 🖱️ **Basic Skills** — Mouse/Keyboard practice เด็กเล็ก

### แหล่งทางการ (Official Sources)
- **Project 14 สสวท.** (12 ชั้น × video + book)
- **CodingThailand** (ป.1-ม.3)
- **DLTV** (แผนรายชั่วโมง + วิดีโอครูสอน)
- **คลังสื่อ.ไทย** (OBEC)
- **My IPST Basket**

### Auto-award P
ทุกคลิก → บันทึก fun click → คะแนน P อัปเดตตาม targetUnits

### ไฟล์
- `src/data/learningResources.ts`
- `src/pages/Resources.tsx`

---

## 🔍 14. Global Search

### Features
- ⌨️ **Ctrl+K** shortcut (Spotlight style)
- 🎯 ค้นใน: units, indicators, topics, resources, games, articles
- 🏷️ Type tags (สี): หน่วย/ตัวชี้วัด/แหล่งเรียนรู้/เกม
- ⌨️ Keyboard nav (↑↓ Enter Esc)

### ไฟล์
- `src/services/searchService.ts` — fuzzy search + scoring
- `src/components/SearchBar.tsx` — UI (Portal-based)

---

## 🛡️ 15. Admin Dashboard (31 เมนู)

| หมวด | เมนู |
|-----|---------|
| การสอนวันนี้ (1) | คาบเรียนวันนี้ |
| หน้าหลักและสถิติ (4) | ภาพรวมระบบ, ห้องเรียน 3D, สถิตินักเรียนในเว็บ, พัฒนาการรายคน |
| ชั้นเรียนและเช็กชื่อ (4) | จัดการนักเรียน, ผู้ทดลองภายนอก, เช็กชื่อตามตาราง, เช็กชื่อ Quick |
| การวัดผล (8) | เก็บคะแนน K/P/A, ส่งออก ปพ.5 & เอกสารวัดผล, แบบประเมินและหลังสอน, คลังข้อสอบ, ทักษะอาชีพ, Bonus, คำถามประจำวัน, งานวิจัย WBI |
| จัดการบทเรียน (7) | จัดการรายวิชา, แผนเทคโนโลยี ป.1, กำหนดการสอน ป.1-6, แผนข้อ 5 ป.1-6, ปลดล็อกบทเรียน, จัดการสไลด์, จัดการตารางสอน |
| สื่อสารและงาน (3) | ประกาศข่าวสาร, ปฏิทินกิจกรรม, การบ้าน |
| ตั้งค่าระบบ (4) | ธีมและสำรองข้อมูล, ประวัติการแก้ไข, ข้อมูลเว็บ, Error Log |

### ระบบส่งออกผลการเรียน ปพ.5 และสเปรดชีต (Grade Export & Statistics Portal)

- บริการ `gradeExportService.ts` คำนวณสถิติทางการ ($\bar{X}$, S.D., แจกแจงเกรด 0-4, ร้อยละเกรด $\ge 3.00$, อัตราการผ่าน)
- ส่งออกไฟล์ Spreadsheet (.xls / .csv) จัดกลุ่มหัวตารางคะแนนเก็บ K/P/A, สอบกลาง/ปลายภาค, รวม 100, เกรด, ผลการประเมิน
- ส่งออก Master Grade Sheet รวม 9 ห้อง 111 คนในไฟล์เดียวสำหรับฝ่ายวิชาการ
- ส่งออกไฟล์นำเข้าตรงตามระบบทะเบียนโรงเรียน (SchoolMIS / SGS)
- หน้าพิมพ์ `OfficialPp5PrintLayout.tsx` ขนาด A4 แนวนอน พร้อมช่องลงนาม 3 ฝ่าย (ครูผู้สอน, หัวหน้างานวัดผล, ผอ.โรงเรียน)
- หน้าพิมพ์ `StudentGradeSlipPrintLayout.tsx` ใบแจ้งผลการเรียนรายบุคคล (ปพ.6) จัดพิมพ์ 2 คน/แผ่น A4 มีรอยปรุตัดแจกผู้ปกครอง
- นโยบายความปลอดภัยของข้อมูล: ทำงานแบบ Read-Only และ export โดยไม่มีการลบหรือดัดแปลงข้อมูลจริงในฐานข้อมูล

### งานวิจัยในชั้นเรียน WBI + ADDIE

- เปิดต้นฉบับบทคัดย่อและบทที่ 1-5 ได้ทันที แล้วเลือกอ่าน/แก้ไขรายบทในหน้า Admin
- บทที่ 4 ใช้ข้อมูล K/P/A, E1/E2, คู่คะแนนก่อน-หลัง, การมีส่วนร่วม และแบบสอบถามที่ระบบมีจริง
- รายงานจำนวนตามบัญชีรายชื่อ นักเรียนที่มีคะแนน และระเบียนผลการเรียนแยกกัน ไม่สร้างค่าคุณภาพเครื่องมือหรือผลที่ยังไม่มี
- การซิงก์ Firebase มีเวลาเผื่อสูงสุด 12 วินาที หากไม่สำเร็จจะคงฉบับจาก local cache ให้ครูทำงานต่อ
- คัดลอก พิมพ์ ดาวน์โหลด และให้ AI เรียบเรียงจากต้นฉบับรวมฉบับเดียวกัน

### บันทึกหลังสอนในหน้าแผน ป.1

- วันที่เริ่มต้นมาจาก `TeachingSession.plannedDate` ของแผนนั้น และเลือกบันทึกตรงวันก่อนฉบับล่าสุด
- ส่วนที่ 12 และฟอร์ม Firebase ใช้ `LessonRecord` ระเบียนเดียวกัน จึงไม่แสดงช่องเส้นประซ้ำเหนือข้อมูลจริง
- ข้อความร่างสร้างจากชื่อเรื่อง จุดประสงค์ กิจกรรม และเครื่องมือของแต่ละแผน แต่ครูต้องตรวจเช็กชื่อ ชิ้นงาน และ K/P/A ก่อนเปลี่ยนสถานะเป็นสมบูรณ์
- ช่องที่ไม่มีหลักฐานแสดงขีด/รอดึงข้อมูล ไม่ตีความเลขศูนย์เป็นผลไม่ผ่าน

### ไฟล์
- `src/pages/AdminDashboard.tsx`
- `src/components/TeacherClassroomHub.tsx`
- `src/components/ResearchGenerator.tsx`
- `src/services/researchService.ts`
- `src/components/{StudentManager, GradeBook, HomeworkManager, QuestionBankManager, AuditLogViewer}.tsx`

---

## 🐛 16. Error Tracking

### Features
- เก็บ error 50 รายการล่าสุดใน localStorage
- Global handler — `window.error` + `unhandledrejection`
- Admin tab "Error Log" — ดูได้ครู

### ไฟล์
- `src/services/errorLogger.ts`
- `src/components/ErrorBoundary.tsx` — React error boundary

---

## 🔥 17. Firebase Security

### Firestore Rules (`firestore.rules`)
- รองรับ `teacherProfiles` และบทบาทครูผ่าน Firebase Auth
- เพิ่ม validation รูปร่างและขนาดข้อมูลใน collections หลัก
- รองรับ collections ของคาบเรียน หลักฐาน K/P/A คลังข้อสอบ เวอร์ชัน และ audit log
- ระบบเดิมบาง collection ยังเปิดตาม shape-based rules เพื่อรองรับ legacy client
- ก่อนใช้งานข้อมูลจริงสาธารณะต้องเปิด App Check, ย้ายบัญชีครูเข้า Firebase Auth
  และปิด legacy fallback ตาม `SECURITY.md`

### Deploy
```bash
firebase deploy --only firestore:rules
```

---

## 📐 18. Score Calculation Detail

### per Indicator (มี N ตัวชี้วัด)
```
น้ำหนัก = 70 / N
K = (student.k / ind.maxScore) × weight × 0.60
P = (P_POINTS[student.p] / 3) × weight × 0.25
A = (student.a ? 1 : 0) × weight × 0.15

ตัวอย่าง ป.5 (5 ตัวชี้วัด):
น้ำหนัก/ตัว = 14 → K=8.4, P=3.5, A=2.1
```

### Total
```
รวมจากทุกตัวชี้วัด: K_total + P_total + A_total = 70 (max)
+ สอบกลางภาค (15 สำหรับ ม.) + ปลายภาค (30 ป. / 15 ม.) = 30
= 100 คะแนน
```

### เกรด
| คะแนน | เกรด |
|------|------|
| ≥ 80 | 4 |
| 75-79 | 3.5 |
| 70-74 | 3 |
| 65-69 | 2.5 |
| 60-64 | 2 |
| 55-59 | 1.5 |
| 50-54 | 1 |
| < 50 | 0 |

---

## 🎨 19. UI/UX Design System

### Color Palette
- **Primary**: `#FFD43B → #FAB005` (gradient yellow)
- **Secondary**: `#1D1D1F` (dark text)
- **Accent**: `#6366f1` (indigo for actions)
- **Success/Warn/Error**: green/amber/red standards

### Typography
- **Display**: Outfit (English) + Sarabun (Thai)
- **Body**: Prompt + Sarabun
- **Code**: JetBrains Mono

### Components
- `.btn-primary` — yellow gradient + shadow
- `.btn-secondary` — white + border
- `.card` — soft shadow + hover lift
- `.section-header` — centered with subtitle
- Modal: Portal-based with `z-index: 10000`

### Responsive Breakpoints
- > 1024px: Desktop full
- ≤ 1024px: Drawer menu
- ≤ 900px: Search/Achievements icon-only
- ≤ 768px: Compact navbar
- ≤ 480px: Logo icon-only

---

## 📦 20. Tech Architecture

### Folder Structure
```
src/
├── pages/           # หน้าหลักของเว็บ
│   ├── games/       # เกมจาก gamesCatalog
│   └── AdminDashboard.tsx (เมนูครูแบบแบ่งหมวด)
├── components/      # UI components ที่ใช้ซ้ำ
├── services/        # Business logic + data CRUD
├── data/            # Static data + content
├── context/         # AuthContext
└── main.tsx         # Entry point
```

### Storage Strategy
**localStorage Keys:**
- `current_student`, `current_partner` — auth
- `krujames_grades_v1_{classroom}[_subj]` — K/P/A
- `krujames_roster_overrides_v1` — custom roster
- `krujames_achievements_{studentId}` — badges
- `krujames_announcements_v1`, `krujames_calendar_v1`, `krujames_schedule_v1`
- `krujames_errors_v1` — error log
- `krujames_custom_courses_v1` — Course Builder

### Code Splitting
ทุก page ใช้ `React.lazy()` → first paint เร็ว

### Build
```bash
npm run dev      # development
npm run build    # production
npm run preview  # preview build
```

---

## 🚀 21. Deployment

### Vercel/Netlify
```bash
npm run build
# Upload dist/ folder
```

### Firebase Hosting
```bash
firebase deploy
firebase deploy --only firestore:rules
```

### Environment Variables (.env)
```
VITE_FIREBASE_API_KEY=...
VITE_FIREBASE_AUTH_DOMAIN=...
VITE_FIREBASE_PROJECT_ID=...
VITE_FIREBASE_STORAGE_BUCKET=...
VITE_FIREBASE_MESSAGING_SENDER_ID=...
VITE_FIREBASE_APP_ID=...
```

ถ้าไม่ตั้ง env → ส่วนที่ใช้ Firebase จะไม่บันทึกขึ้น cloud และไม่เหมาะกับการใช้งานจริง

---

## 🔐 22. Admin Access

- URL: `/admin`
- Session ฝั่ง browser: 8 ชั่วโมง
- ห้ามบันทึกรหัสผ่านจริงในเอกสารหรือ repository
- ระบบปัจจุบันเป็น client-side gate จึงอ่านค่าใน bundle ได้ ดูแผนแก้ไขใน `SECURITY.md`

---

## 🎯 23. Roadmap (สิ่งที่ทำต่อได้)

### High Priority
- 🌙 Dark mode toggle
- 📱 PWA (install เป็นแอป + offline)
- 📨 Contact/Feedback form ติดต่อครู
- ❓ FAQ/Help page

### Medium Priority
- 🎯 Goal Setting (เป้านักเรียนรายสัปดาห์)
- 📝 Note Taking ในแต่ละหน่วย
- 👥 Class Forum / Q&A
- 🌐 i18n (English support)
- 🤝 Parent Portal (QR code)

### Technical
- ✅ Unit tests (Vitest) — 154 tests ผ่านในรุ่น `589b85e`
- 📊 Real analytics (Plausible)
- 🔒 Stricter Firestore rules
- 🚀 SSR (Next.js migration?)

---

## 📊 24. สถิติระบบ

| รายการ | จำนวน |
|-------|------|
| คอร์สเรียน | **16+** (ป.1-ม.3 + AI 3 ระดับ + custom) |
| หน่วยการเรียน | **70+** หน่วย |
| ตัวชี้วัด | **50+** (ว 4.1 + ว 4.2) |
| สไลด์เนื้อหา | **500+** สไลด์ (JPG + Rich) |
| Rich Slides | **426 สไลด์** ครบ 73 หน่วย พร้อมภาพ เสียงอ่าน และกิจกรรมสั้น |
| แบบทดสอบ | **200+ ข้อ** |
| Resources (เครื่องมือ) | **107+** รายการ |
| เกมในเว็บ | **24 เกมใน catalog** |
| Achievements | **18 badges** |
| รายชื่อนักเรียน 2569 | **111 คน** 9 ห้อง |
| Pages | **17 pages** |
| Components | **20 components** |
| Services | **18 services** |
| Admin | **เมนูแบบแบ่งหมวด ดูรายการจริงจาก `AdminDashboard.tsx`** |

---

## 📝 ผู้พัฒนา

**ครูเจมส์ (Kru James Soncom)**  
โรงเรียนบ้านคลองมดแดง  
สำหรับเรียน วิชาเทคโนโลยี (วิทยาการคำนวณ + การออกแบบเทคโนโลยี)  
ปีการศึกษา 2569

---

*เอกสารนี้สร้างอัตโนมัติจากระบบ — อัปเดตล่าสุด: ปีการศึกษา 2569*
